...
from models_sdp import BinarizeConv2dSDP
...
model_names = sorted(name for name in models_sdp.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(models_sdp.__dict__[name]))

parser = argparse.ArgumentParser(description='PyTorch ConvNet Training')

parser.add_argument('--results_dir', metavar='RESULTS_DIR', default='./results_sdp',
                    help='results dir')
...
def main():
    ...
    model = models_sdp.__dict__[args.model]
