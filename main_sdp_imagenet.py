...
# import torchvision.models_imagenet_sdp as models_imagenet_sdp
import models_imagenet_sdp
import logging

from models_imagenet_sdp import BinarizeConv2dSDP
import copy
...
def parse():
    model_names = sorted(name for name in models_imagenet_sdp.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(models_imagenet_sdp.__dict__[name]))
...
def main():
    ...
    #     model = models_imagenet_sdp.__dict__[args.arch](pretrained=True)
    # else:
    #     print("=> creating model '{}'".format(args.arch))
    #     model = models_imagenet_sdp.__dict__[args.arch]()
    if args.local_rank == 0:
